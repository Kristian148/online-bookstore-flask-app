...
def update_cart():
    ...
    If quantity <= 0, the book is removed from the cart.
    """
    book_title = request.form.get('title')
    quantity = int(request.form.get('quantity', 1))

    # Fix: actually remove the book when quantity is 0 or less
    if quantity <= 0:
        cart.remove_book(book_title)
        flash(f'Removed \"{book_title}\" from cart!', 'success')
    else:
...
def register():
    ...
    if request.method == 'POST':
        ...
        # Add this new email format validation
        import re
        email_pattern = r"[^@]+@[^@]+\.[^@]+"
        ...
        if email in [u.lower() for u in users.keys()]:
